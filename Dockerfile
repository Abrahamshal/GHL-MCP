...
FROM base AS build
RUN --mount=type=cache,id=npm,target=/root/.npm npm ci --no-audit --prefer-offline
COPY . .
RUN npm run build
...
RUN --mount=type=cache,id=npm,target=/root/.npm npm ci --omit=dev --no-audit --prefer-offline
